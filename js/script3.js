...
let cardsBlock = $("#cards1");
function drawCard(title, description, price, imageUrl, podCategory) {
  cardsBlock.append(`
  <div class="card">
  ...
  <div class="info__block">
    ...
    <div class="title__city">
      ...
      <span class="category"> ${podCategory} </span>
      <button class="add" onclick = "addToCart('${title}', ${price}, '${description}')">В корзину</button>
    </div>
  </div>
</div>
    `)
...
$("#button").click(function () {
  ...
  for (let c in items1) {
    if (c === category) {
      for (let i = 0; i < items1[c].length; i++) {
        if (items1[c][i]['podCategory'] === subCategory && Number(items1[c][i]['price']) < maxPrice) {
          drawCard(
            items1[c][i]['title'],
            items1[c][i]['description'],
            items1[c][i]['price'],
            items1[c][i]['imageUrl'],
            items1[c][i]['podCategory']
          );
        }
...
class Item extends Base {
  _description
  _imageUrl;
  constructor(newName, newPrice, newDescription, newImage) {
    super(newName, newPrice)
    this._description = newDescription
    this._imageUrl = newImage
  }
...
function addToCart(imageUrl, name, price, description) {
  // логика добавления item в корзину

  let item = new Item(imageUrl, name, price, description)
